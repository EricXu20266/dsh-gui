...
export function AboutSection({ t }: AboutSectionProps): ReactNode {
  ...
  useEffect(() => {
    ...
    fetch('/dsh-about/info')
      .then((res) => { if (!res.ok) throw new Error(String(res.status)); return res.json() as Promise<AboutInfo> })
      .then((data) => { if (alive) { setInfo(data); setError(false) } })
      .catch(() => { if (alive) setError(true) })
    return () => { alive = false }
  }, [])

  if (error) {
    return <div style={rootStyle}><span style={rowLabelStyle}>{t('loadError')}</span></div>
  }
  if (info === null) {
    return <div style={rootStyle}><span style={rowLabelStyle}>{t('loading')}</span></div>
  }
